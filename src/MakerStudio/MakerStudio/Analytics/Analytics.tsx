...
const KPICard: React.FC<KPICardProps> = ({ title, value, trend, positive, icon }) => (
...
const ResponsiveLineChart: React.FC<ResponsiveLineChartProps> = ({ data, labels, color }) => {
    ...
    <div className={styles.lineChartWrapper}>
      ...
      <svg viewBox="0 0 100 100" preserveAspectRatio="none" className={styles.svgChart}>
        ...
        {data.map((_, i) => {
          ...
          return (
            <circle key={i} cx={x} cy={y} r="4" fill="transparent"
      ...
      <div className={styles.xLabels}>
        {labels.map((label, i) => (
          <span key={i} className={`${styles.xLabel} ${i === 0 || i === labels.length - 1 ? styles.visible : styles.hideMobile}`}>{label}</span>
        ))}
      </div>
    </div>
  );
...
const CustomDonutChart: React.FC = () => (
  <div className={styles.donutChart}>
    ...
    <div className={styles.donutCenter}><span>100%</span></div>
  </div>
);
  ...
  return (
    <div className={styles.container}>
      <div className={styles.header}>
        <div className={styles.headerLeft}><h1>Advanced Analytics</h1><p>Real-time infrastructure performance metrics.</p></div>
        <div className={styles.headerControls}>
          <button type="button" className={styles.exportBtn} onClick={() => alert("Downloading CSV...")}>
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="7 10 12 15 17 10"/><line x1="12" y1="15" x2="12" y2="3"/></svg>
            <span>Export</span>
          </button>
          <div className={styles.periodToggle}>
            {['7d', '30d', '12m'].map(p => (
              <button key={p} type="button" className={`${styles.toggleBtn} ${period === p ? styles.active : ''}`} onClick={() => setPeriod(p)}>
      ...
      {lowStockItems.length > 0 && (
        <div className={styles.alertBanner}>
          <div className={styles.alertIcon}><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M10.29 3.86L1.82 18a2 2 0 0 0 1.71 3h16.94a2 2 0 0 0 1.71-3L13.71 3.86a2 2 0 0 0-3.42 0z"/><line x1="12" y1="9" x2="12" y2="13"/><line x1="12" y1="17" x2="12.01" y2="17"/></svg></div>
          <div className={styles.alertContent}><strong>Restock Needed:</strong> <span>{lowStockItems.map(i => i.name).join(', ')} are running low.</span></div>
          <button type="button" className={styles.alertAction} onClick={() => alert("Navigate to Products")}>Manage Stock</button>
        </div>
      )}

      <div className={styles.kpiGrid}>
        <KPICard title={viewMode === 'revenue' ? "Total Revenue" : "Total Profit"} value={fmt(displayValue)} trend="+12.5%" positive={true} icon={<CurrencyIcon />} />
        <KPICard title="Avg Order Value" value={fmt(avgOrderValue)} trend="+3.2%" positive={true} icon={<CartIcon />} />
        <KPICard title="Conversion Rate" value={`${conversionRate}%`} trend="-0.4%" positive={false} icon={<EyeIcon />} />
        <KPICard title="Total Orders" value={totalOrders.toLocaleString()} trend="+8.1%" positive={true} icon={<BoxIcon />} />
      </div>
      ...
      <div className={styles.chartSection}>
        <div className={styles.sectionHeader}>
          ...
          <div className={styles.viewToggle}>
            <button type="button" className={`${styles.viewBtn} ${viewMode === 'revenue' ? styles.active : ''}`} onClick={() => setViewMode('revenue')}>Revenue</button>
            <button type="button" className={`${styles.viewBtn} ${viewMode === 'profit' ? styles.active : ''}`} onClick={() => setViewMode('profit')}>Profit</button>
          </div>
        </div>
        <div className={styles.chartContainer}>
          <ResponsiveLineChart data={viewMode === 'revenue' ? data.revenue : data.revenue.map((r, i) => r - data.cost[i])} labels={data.labels} color="var(--brut-text)" />
        </div>
      </div>

      <div className={styles.twoColumn}>
        <div className={styles.section}>
          <div className={styles.sectionHeader}><h3>Top Performing Products</h3><button type="button" className={styles.viewAll} onClick={() => alert("View Full Report")}>View Report</button></div>
            ...
            <table className={styles.table}>
              ...
              <tbody>
                {topProducts.map(product => (
                  <tr key={product.id}>
                    <td><div className={styles.productCell}><div className={styles.thumb}></div><span>{product.name}</span></div></td>
                    <td>{product.sales}</td>
                    <td>${product.revenue.toLocaleString()}</td>
                    <td><span className={`${styles.growth} ${product.growth >= 0 ? styles.positive : styles.negative}`}>{product.growth > 0 ? '+' : ''}{product.growth}%</span></td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <div className={styles.section}>
          <div className={styles.sectionHeader}><h3>Traffic Sources</h3></div>
          <div className={styles.donutContainer}>
            <CustomDonutChart />
            <div className={styles.donutLegend}>
              <div className={styles.legendRow}><span className={styles.colorDot} style={{ background: '#3b82f6' }}></span><span>Direct (45%)</span></div>
              <div className={styles.legendRow}><span className={styles.colorDot} style={{ background: '#8b5cf6' }}></span><span>Social (30%)</span></div>
              <div className={styles.legendRow}><span className={styles.colorDot} style={{ background: '#f59e0b' }}></span><span>Search (25%)</span></div>
            </div>
          </div>
        </div>
      </div>
